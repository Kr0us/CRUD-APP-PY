...
def insertar_cliente_pedido():
    ...
    for i, label in enumerate(pedido_labels):
        ...
        if label == "Tipo Despacho":
            entry = ttk.Combobox(form_window, values=tipo_despacho, width=28)   
        elif label == "Fecha":
            entry = DateEntry(form_window, width=30, date_pattern='dd-mm-yyyy')
        elif label == "Pago":
            entry = ttk.Combobox(form_window, values=pagos, width=28)
        else:
...
def load_table(tabla):
    ...
    if tabla == 'CLIENTE':
        ...
        for i, col in enumerate(columns):
            ttk.Label(form_frame, text=col).grid(row=i, column=0, padx=5, pady=5, sticky='W')
            if col == "Tipo_Despacho":
                entry = ttk.Combobox(form_frame, values=tipo_despacho, width=28)
            elif col == "Fecha":
                entry = DateEntry(form_frame, width=30, date_pattern='dd-mm-yyyy')
            elif col == "Pago":
                entry = ttk.Combobox(form_frame, values=pagos, width=28)    
            else:
                entry = ttk.Entry(form_frame, width=30)
            entry.grid(row=i, column=1, padx=5, pady=5, sticky='W')
            if col == "ID_Pedido":
...
def main_window():
    ...
    btn_admin_pedido_santiago = ttk.Button(top_frame, text="Pedido Santiago", command=admin_pedido_santiago)
    btn_admin_pedido_starken = ttk.Button(top_frame, text="Pedido Starken", command=admin_pedido_starken)
